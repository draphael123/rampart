...
export class Enemy {
    ...
    if (this.stun > 0) {
      ...
      switch (this.state) {
        ...
        case 'chase': {
          ...
          if (canAttack && this.game.requestAttackToken(this)) { this.state = 'windup'; this.t = 0; this.emit('windup'); }
          else if (!this.perch && d > c.stop) {
            const f = this.fwd();
            // don't walk off ledges: probe ahead
            const ahead = { x: this.pos.x + f.x * 0.9, y: this.pos.y - 1.5, z: this.pos.z + f.z * 0.9 };
            const probe = { min: { x: ahead.x - 0.2, y: ahead.y, z: ahead.z - 0.2 }, max: { x: ahead.x + 0.2, y: this.pos.y + 0.3, z: ahead.z + 0.2 } };
            const floor = this.world.near(probe, 0).some(bx => bx.max.y <= this.pos.y + 0.5 && bx.max.y >= this.pos.y - 1.6 && overlapXZ(probe, bx));
            if (floor || !b.grounded) move = { x: f.x, z: f.z };
          }
        ...
        case 'windup':
          ...
          if (this.t >= c.windup) { this.state = 'swing'; this.t = 0; this.hitDone = false; this.emit('swing'); }
          break;
        case 'swing':
          this.telegraph = 1;
          if (!this.hitDone) {
          ...
          if (this.t >= c.swing) { this.state = 'recover'; this.t = 0; this.attackCount++; this.game.releaseAttackToken(this); }
          break;
        case 'recover':
          this.telegraph = 0;
  ...
  takeHit(dmg, fromPos, opts = {}) {
    ...
    if (!(this.kind === 'captain' && opts.kind === 'light')) {
